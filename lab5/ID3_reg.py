...
class ID3RegressionTreePredictor:
    ...
    def calcMSE(self, dataIDXs):
        mse = 0.0
        avg = 0.0
        avg = np.mean(list(self.__target[i] for i in dataIDXs))
        mse = np.sum((self.__target[i] - avg) ** 2 for i in dataIDXs)
        return mse, avg
    ...
    def findSplitAttr(self, attributes, dataIDXs):
        ...
        attributeList = list(self.__attributes)

        mse_values = []

        for a in attributes:
            a_idx = attributeList.index(a)
            oMSE = 0

            for sub_a in self.__attributes[a]:
                idxs = [i for i in dataIDXs if self.__data[i][a_idx] == sub_a]
                mse, avg = self.calcMSE(idxs)
                oMSE += mse
            mse_values.append(oMSE)

        best_split_index = np.argmin(mse_values)
        minMSE = best_split_index
        splitAttr = attributes[best_split_index]
        a_idx = attributeList.index(splitAttr)

        for sub_a in self.__attributes[splitAttr]:
            idxs = [i for i in dataIDXs if self.__data[i][a_idx] == sub_a]
            mse, avg = self.calcMSE(idxs)
            splitMSEs[sub_a] = mse
            splitDataIDXs[sub_a] = idxs
            splitAverages[sub_a] = avg
    ...
    def fit_rek(self, depth, parentID, splitVal, attributesToTest, mse, avg, dataIDXs):
        ...
        if (
            currentDepth == self.__maxDepth
            or mse <= self.__stopMSE
            or len(attributesToTest) == 0
            or len(dataIDXs) < self.__minSamplesSplit
        ):
            root.update({"avgValue": avg})
            self.addNodeToGraph(root, parentID)
            return root

        minMSE, splitAttr, splitMSEs, splitAverages, splitDataIDXs = self.findSplitAttr(
            attributesToTest, dataIDXs
        )
        ...
        for val in self.__attributes[splitAttr]:
            ...
            if len(splitDataIDXs[val]) < self.__minSamplesLeaf:
                ...
                root["nodes"][val].update(
                    {
                        "splitValue": val,
                        "samples": len(splitDataIDXs[val]),
                        "avgValue": splitAverages[val],
                    }
                )
                self.addNodeToGraph(root["nodes"][val], root["id"])
                print(
                    "leaf, not enough samples, setting node-value = "
                    + str(splitAverages[val])
                )

            else:
                root["nodes"][val] = self.fit_rek(
                    currentDepth + 1,
                    root["id"],
                    val,
                    attributesToTestCopy,
                    splitMSEs[val],
                    splitAverages[val],
                    splitDataIDXs[val],
                )
    ...
    def predict(self, data):
        ...
        for i in range(len(data)):
            s = self.predict_rek(data[i], self.__tree)
            if np.isnan(s):
                s = 0
            predicted.append(s)
    ...
    def score(self, data, target):
        ...
        y_pred = np.array(self.predict(data))
        y_true = np.array(target)
        y_true_mean = np.mean(y_true)

        u = np.sum((y_true - y_pred) ** 2)  # Residual sum of squares
        v = np.sum((y_true - y_true_mean) ** 2)  # Total sum of squares

        score = 1.0 - u / v  # Coefficient of determination of the preidction: R^2
